...
trait Ticket_Maker {
	 ...
	 */
	protected function create_paypal_ticket( int $post_id, int $price = 1, array $overrides = [] ) {
	...
	protected function create_distinct_paypal_tickets( int $post_id, array $tickets, $global_qty = 0 ) {
		...
		foreach ( $tickets as $ticket ) {
			...
			$price = $ticket[ 'price' ] ?? random_int( 1, 5 );

			// Create ticket.
			$ticket_id    = $this->create_paypal_ticket( $post_id, $price, $ticket );
			$ticket_ids[] = $ticket_id;

			// Handle tickets with global stock
			$mode = $ticket['meta_input'][ $global_stock::TICKET_STOCK_MODE ] ?? $global_stock::OWN_STOCK_MODE;
			if ( $global_stock::OWN_STOCK_MODE !== $mode ) {
