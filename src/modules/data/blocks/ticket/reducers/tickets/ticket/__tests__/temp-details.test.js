...
describe( 'Temp details reducer', () => {
	...
	it( 'should set the temp title', () => {
		expect( reducer( DEFAULT_STATE, actions.setTicketTempTitle( 'block-id', 'new title' ) ) ).toMatchSnapshot();
	} );

	it( 'should set the temp description', () => {
		expect( reducer( DEFAULT_STATE, actions.setTicketTempDescription( 'block-id', 'new description' ) ) ).toMatchSnapshot();
	} );

	it( 'should set the temp price', () => {
		expect( reducer( DEFAULT_STATE, actions.setTicketTempPrice( 'block-id', 99 ) ) ).toMatchSnapshot();
	} );
	...
	it( 'should set the temp end date', () => {
		expect( reducer( DEFAULT_STATE, actions.setTicketTempEndDate( 'block-id', 'January 1, 2018' ) ) ).toMatchSnapshot();
	} );
	...
	it( 'should set the temp start time', () => {
		expect( reducer( DEFAULT_STATE, actions.setTicketTempStartTime( 'block-id', '13:45' ) ) ).toMatchSnapshot();
	} );
	...
	it( 'should set the temp capacity', () => {
		expect( reducer( DEFAULT_STATE, actions.setTicketTempCapacity( 'block-id', 20 ) ) ).toMatchSnapshot();
	} );
} );
