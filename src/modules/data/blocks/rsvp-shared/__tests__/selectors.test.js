 ...
 */
import { selectors } from '@moderntribe/tickets/data/blocks/rsvp-shared';
import { DEFAULT_STATE } from '@moderntribe/tickets/data/blocks/rsvp-shared/reducer';
...
describe( 'RSVP block selectors', () => {
	...
	it( 'should return the inventory', () => {
		expect( selectors.getRSVPInventory( state ) )
			.toBe( DEFAULT_STATE.inventory );
	} );

	it( 'should prefer inventory for available tickets when set', () => {
		...
		expect( selectors.getRSVPAvailable( inventoryState ) ).toBe( 12 );
	} );
