...
export function* setRSVPDetails( action ) {
	...
	yield all( [
		...
		put( actions.setRSVPShowAttendees( showAttendees ) ),
...
export function* setRSVPTempDetails( action ) {
	...
	yield all( [
		...
		put( actions.setRSVPTempShowAttendees( tempShowAttendees ) ),
...
export function* syncRSVPSaleEndWithEventStart( prevStartDate ) {
	try {
		...
		if ( isNotManuallyEdited && isSyncedToEventStart ) {
			...
			yield put( { type: types.RSVP_SALE_END_SYNCED_TO_EVENT_START } );
		}
	} catch ( error ) {
		// ¯\_(ツ)_/¯
		console.error( error );
