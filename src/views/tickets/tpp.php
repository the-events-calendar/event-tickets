...
?>
	...
	<form
		id="tpp-buy-tickets"
		action="<?php echo esc_url( $commerce->get_cart_url() ); ?>"
	...
	>
		...
		<table class="tribe-events-tickets tribe-events-tickets-tpp">
			...
			<?php if ( $is_there_any_product_to_sell ) : ?>
				<tr>
					<td colspan="4" class="tpp-add">
						<?php if ( $must_login ): ?>
							<?php include Tribe__Tickets_Plus__Main::instance()->get_template_hierarchy( 'login-to-purchase' ); ?>
						<?php else: ?>
							<button type="submit" class="tpp-submit tribe-button"><?php esc_html_e( 'Add to cart', 'event-tickets' );?></button>
						<?php endif; ?>
					</td>
				</tr>
			<?php endif ?>
