...
class Tribe__Tickets__REST__V1__Post_Repository implements Tribe__Tickets__REST__Interfaces__Post_Repository {
	...
	public function __construct( Tribe__REST__Messages_Interface $messages = null ) {
		$this->types_get_map = array(
			Tribe__Tickets__Main::POSTTYPE            => array( $this, 'get_event_data' ),
			Tribe__Tickets__Main::VENUE_POST_TYPE     => array( $this, 'get_venue_data' ),
			Tribe__Tickets__Main::ORGANIZER_POST_TYPE => array( $this, 'get_organizer_data' ),
		);
	...
	protected function add_global_id_fields( $data, $post_id ) {
		...
		$lineage = get_post_meta( $post_id, Tribe__Tickets__Aggregator__Event::$global_id_lineage_key );
	...
	protected function get_categories( $event_id ) {
		$data = $this->get_terms( $event_id, Tribe__Tickets__Main::TAXONOMY );
	...
	public function prepare_terms_data( array $terms_data, $taxonomy ) {
		...
		foreach ( $terms_data as $term_data ) {
			...
			$namespace_map = array(
				Tribe__Tickets__Main::TAXONOMY => 'categories',
				'post_tag'                    => 'tags',
			);
	...
	public function prepare_term_data( $term_data, $taxonomy, $namespace ) {
		...
		$term_data['urls'] = array(
			'self'       => tribe_tickets_rest_url( "{$namespace}/{$term_id}" ),
			'collection' => tribe_tickets_rest_url( $namespace ),
		);

		if ( ! empty( $term_data['parent'] ) ) {
			$term_data['urls']['up'] = tribe_tickets_rest_url( "{$namespace}/{$term_data['parent']}" );
		}
