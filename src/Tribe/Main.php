...
class Tribe__Tickets__Main {
	...
	*/
	...
	public function tec_compatibility_notice() {
		...
		$version = str_replace( '-dev', '', $this->min_tec_version );  
		$output .= '<p>' . sprintf( __( 'When The Events Calendar and Event Tickets are both activated, The Events Calendar must be running version %1$s or greater. Please %2$supdate now.%3$s', 'event-tickets' ), $version, '<a href="' . esc_url( $upgrade_path ) . '">', '</a>' ) . '</p>';
		$output .= '</div>';
