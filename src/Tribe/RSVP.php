	...
	public function generate_tickets( ) {
		...
		foreach ( (array) $_POST['product_id'] as $product_id ) {
			...
			for ( $i = 0; $i < $qty; $i ++ ) {
				...
				do_action ( 'tickets_rsvp_tickets_created', $attendee_id, $event_id, $product_id );
			}
			
			do_action( 'tickets_rsvp_tickets_generated_for_product', $product_id, $order_id, $qty );
		}
		
		do_action ( 'tickets_rsvp_tickets_generated', $order_id );
