...
class ScriptLoader {
	...
	public function loadAdminScripts() {
		...
		wp_localize_script( 'give-paypal-partner-js', 'givePayPalCommerce', [
				'translations' => [
					...
					'pciWarning'                        => sprintf( __( 'PayPal allows you to accept credit or debit cards directly on your website. Because of
							...
							seamlessly with SSL certificates. Compliance is comprised of, but not limited to:', 'event-tickets' ), 'https://givewp.com/documentation/resources/pci-compliance/' ),
					...
					'liveWarning'                       => give_is_test_mode() ? esc_html__( 'You have connected your account for test mode. You will need to connect again once you
						are in live mode.', 'event-tickets' ) : '',
				],
			] );
...
EOT;
	...
	public function loadPublicAssets() {
		...
		wp_localize_script( $scriptId, 'givePayPalCommerce', [
				...
				'textForOverlayScreen'                  => sprintf( '<h3>%1$s</h3><p>%2$s</p><p>%3$s</p>', esc_html__( 'Donation Processing...', 'event-tickets' ), esc_html__( 'Checking donation status with PayPal.', 'event-tickets' ), esc_html__( 'This will only take a second!', 'event-tickets' ) ),
			] );
	}
	...
	private function getPartnerJsUrl() {
		return sprintf( '%1$swebapps/merchantboarding/js/lib/lightbox/partner.js', tribe( PayPalClient::class )->getHomePageUrl() );
	}
}
