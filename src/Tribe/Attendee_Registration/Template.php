...
class Tribe__Tickets__Attendee_Registration__Template extends Tribe__Templates {
	...
	public function hook() {
		// Block caching - this may be too late (plugins_loaded)?
		$this->cache_busters();
	...
	public function cache_busters() {
		// We only want to do this for the AR page(s)
		if ( ! $this->is_on_ar_page() ) {
			return;
		}

		// For caching plugins that respect the constant
		...
		// set a cookie to (hopefully) tell Varnish to not cache (at least on WPE)
		setcookie('wordpress_tribe_cachebuster', microtime(true), time()+600, '/');
	}
