<?php
/**
 * Class Tribe__Tickets__Attendee_Info__Template
 */
class Tribe__Tickets__Attendee_Info__Template extends Tribe__Templates {
	 ...
	 * template we're gonna use for the attendee-info
	 * page
	 *
	 * @since TBD
	 *
	...
	public function add_body_classes( $classes ) {
		...
		$classes[] = 'page-tribe-attendee-info';
	...
	public function set_page_content( $query ) {
		...
		if ( $this->is_main_loop( $query ) ) {
			...
			add_filter( 'the_content', array( tribe( 'tickets.attendee_info.view' ), 'display_attendee_info_page' ) );
		}
	}
	...
	/**
	 * Return the Attendee Info page title
	 *
	 * @since TBD
	 *
	 * @return string
	...
	public function get_page_title() {
		/**
		 * `tribe_tickets_attendee_info_page_title`
		 * Filters the attendee info page title
		 *
		 * @param array $post_types Array of post types
		 */
		return apply_filters( 'tribe_tickets_attendee_info_page_title', esc_html__( 'Attendee Info', 'event-tickets' ) );
	}

	/**
	 * Check if we're on the attendee info page
	 * Based on the attendee-info query var.
	 *
	 * @since TBD
	 *
	 * @return string
	 */
	private function is_attendee_info_page() {
		global $wp_query;
		...
		return ! empty( $wp_query->query_vars['attendee-info'] );
	}
