...
class Tribe__Tickets__Tickets_Handler {
	...
	public function attendees_page_load_pointers( $hook ) {
		...
		if ( version_compare( get_bloginfo( 'version' ), '3.3', '>' ) && ! in_array( 'attendees_filters', $dismissed ) ) {
			$pointer = array(
				...
				'options'    => array(
					'content' => sprintf( '<h3> %s </h3> <p> %s </p>', __( 'Columns', 'tribe-tickets' ), __( 'You can use Screen Options to select which columns you want to see. The selection works in the table below, in the email, for print and for the CSV export.', 'tribe-tickets' ) ),
	...
	public function maybe_generate_attendees_csv() {
		...
		if ( ! empty( $items ) ) {
			...
			$filename = sanitize_file_name( $event->post_title . '-' . __( 'attendees', 'tribe-tickets' ) );
