...
trait With_Filtered_Ticket_Params {
	...
	public function filter_upsert_params( array $params ): array {
		...
		if ( ! in_array( get_post_type( $event_id ), (array) tribe_get_option( 'ticket-enabled-post-types', [] ), true ) ) {
			...
			// translators: 1) is the name of the parameter.
			$exception->set_details( sprintf( __( 'The parameter `{`%1$s}` does not support ticket creation. Make sure it\'s post type is enabled for tickets under Tickets > Settings > Ticket Settings > Post types that can have tickets.', 'event-tickets' ), 'event' ) );
			throw $exception;
		}
