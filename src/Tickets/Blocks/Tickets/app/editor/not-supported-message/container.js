...
const mapStateToProps = ( state, ownProps ) => {
	let mappedProps = {
		...
		content: sprintf(
			...
			__( 'Standard %s are not yet supported on recurring events.', 'event-tickets' ),
			TICKET_LABELS.ticket.pluralLowercase
		),
		ctaLink: (
			<a className="helper-link" href="https://evnt.is/1b7a" target="_blank" rel="noopener noreferrer">
	 ...
	 */
	mappedProps = applyFilters( 'tec.tickets.blocks.Tickets.NotSupportedMessage.mappedProps', mappedProps, {
		state,
		ownProps,
	} );
...
export default compose( withStore(), connect( mapStateToProps ) )( NotSupportedMessage );
