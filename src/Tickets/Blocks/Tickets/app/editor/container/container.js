...
const getHasOverlay = ( state, ownProps ) =>
	selectors.getTicketsIsSettingsOpen( state ) || ( ! selectors.hasATicketSelected( state ) && ! ownProps.isSelected );
...
const mapStateToProps = ( state, ownProps ) => {
	let mappedProps = {
		...
		uneditableTicketsAreLoading: selectors.getUneditableTicketsAreLoading( state ),
	};
	 ...
	 */
	mappedProps = applyFilters( 'tec.tickets.blocks.Tickets.TicketsContainer.mappedProps', mappedProps, {
		state,
		ownProps,
	} );
