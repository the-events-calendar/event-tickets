...
export const selectors = {
	...
	getLayoutSeats( state, layoutId ) {
		return state.layouts.find( ( layout ) => layout.id === layoutId )?.seats || 0;
	},
	...
	getSeatTypeSeats( state, seatTypeId ) {
		...
		return state?.seatTypesByLayoutId?.[ state.currentLayoutId ]?.[ seatTypeId ]?.seats || 0;
	},
	getTicketSeatType( state, clientId ) {
		...
		return state?.seatTypesByPostId?.[ ticketPostId ] || state?.seatTypesByClientId?.[ clientId ] || null;
	},
