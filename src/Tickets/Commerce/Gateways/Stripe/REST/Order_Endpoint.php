...
class Order_Endpoint extends Abstract_REST_Endpoint {
	...
	/**
	 * Handles the request that updates an order with Tickets Commerce and the Stripe gateway.
	 *
	 * @since TBD
	 *
	 * @param WP_REST_Request $request The request object.
	...
	public function handle_update_order( WP_REST_Request $request ) {
		...
		$stripe_order_id = $request->get_param( 'order_id' );

		$order = tec_tc_orders()->by_args( [
			'status'           => tribe( Pending::class )->get_wp_slug(),
			'gateway_order_id' => $stripe_order_id,
		] )->first();

		if ( ! $order ) {
			return new WP_Error( 'tec-tc-gateway-stripe-nonexistent-order-id', $messages['nonexistent-order-id'], $order );
		}

		$payer_id = $request->get_param( 'payer_id' );

		$stripe_capture_response = tribe( Client::class )->capture_order( $stripe_order_id, $payer_id );

		$debug_header = tribe( Client::class )->get_debug_header();
		if ( ! empty( $debug_header ) ) {
			$stripe_capture_response['debug_id'] = $debug_header;
		}

		if (
			'UNPROCESSABLE_ENTITY' === Arr::get( $stripe_capture_response, 'name' )
		) {
			// Flag the order as Denied.
			tribe( Order::class )->modify_status( $order->ID, Denied::SLUG, [
				'gateway_payload' => $stripe_capture_response,
			] );

			return new WP_Error( 'tec-tc-gateway-stripe-failed-capture', $messages['failed-capture'], $stripe_capture_response );
		}

		$stripe_capture_status = Arr::get( $stripe_capture_response, [ 'status' ] );
		$status                = tribe( Status::class )->convert_to_commerce_status( $stripe_capture_status );

		if ( ! $status ) {
			return new WP_Error( 'tec-tc-gateway-stripe-invalid-capture-status', $messages['invalid-capture-status'], $stripe_capture_response );
		}
		...
		$response['redirect_url'] = add_query_arg( [ 'tc-order-id' => $stripe_order_id ], tribe( Success::class )->get_url() );

		return new WP_REST_Response( $response );
	}

	/**
	 * Handles the request that handles failing an order with Tickets Commerce and the Stripe gateway.
	 *
	 * @since 5.2.0
	 *
	 * @param WP_REST_Request $request The request object.
	...
	public function handle_fail_order( WP_REST_Request $request ) {
		...
		if ( ! $order ) {
			return new WP_Error( 'tec-tc-gateway-stripe-nonexistent-order-id', null, $order );
		}
	...
	/**
	 * Arguments used for the updating order for Stripe.
	 *
	 * @since TBD
	 *
	 * @return array
	...
	public function update_order_args() {
		return [
			...
			'payer_id' => [
				'description'       => __( 'Payer ID token from Stripe', 'event-tickets' ),
				'required'          => false,
				'type'              => 'string',
				'validate_callback' => static function ( $value ) {
					if ( ! is_string( $value ) ) {
	...
	/**
	 * Arguments used for the deleting order for Stripe.
	 *
	 * @since 5.2.0
	 *
	 * @return array
