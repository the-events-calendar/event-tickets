...
class Handler {
	 ...
	 *
	 * @return array|false The `parent_payment` link, or false when there is none.
	 */
	...
	public function process_event( $event ) {
		...
		if ( '' === $gateway_order_id ) {
			...
			/*
			 * Answered as success for the same reason as a settled order below: every redelivery carries
			 * this same payload, so a retry can never resolve an order that this one could not.
			 */
			return new WP_Error(
		...
		if ( ! $order instanceof \WP_Post ) {
			...
			/*
			 * Answered as success: the local order stores its PayPal order id before the buyer can approve
			 * the payment, so an order that is missing now will still be missing on every retry.
			 */
			return new WP_Error(
		...
		if ( $new_status->get_wp_slug() === $order->post_status ) {
			 ...
			 */
			return new WP_Error(
				'tec-tickets-commerce-paypal-webhook-order-status-already-updated',
				null,
				[
					'status'           => 200,
					'gateway_order_id' => $gateway_order_id,
