...
class Merchant {
	...
	/**
	 * @todo Methods below are related ot the Client Token, which will be used for the implementation of Advanced Credit
	 *       and Debit Payments later down the road.
	 *
	 ...
	 */
	/**
	 * Returns the options key for the client token in the merchant mode.
	 *
	 * @since TBD
	 *
	 * @return string
	 */
	public function get_client_token_key() {
		return "tribe_tickets_paypal_commerce_{$this->get_mode()}_client_token";
	}
	...
	public function get_client_token( $force = false ) {
		$option_name = $this->get_client_token_key();

		// If in this request we asked for the token we use it.
		if ( false === $force && ! empty( $this->client_token ) ) {
			return $this->client_token;
		}

		// Try to fetch from the transient.
		if ( false === $force && $this->client_token = get_transient( $option_name ) ) {
			return $this->client_token;
		}

		$this->refresh_client_token();

		return $this->client_token;
	}

	public function refresh_client_token() {
		$option_name = $this->get_client_token_key();

		$response = wp_remote_post(
			tribe( Client::class )->get_api_url( 'v1/identity/generate-token' ),
			[
				'headers' => [
		...
		// @todo Determine error handling for failing response.
		if ( ! array_key_exists( 'client_token', $data ) ) {
			return '';
		}

		// Expire token before one minute to prevent unnecessary race condition.
		set_transient( $option_name, $data['client_token'], $data['expires_in'] - 60 );

		$this->client_token            = $data['client_token'];
		$this->client_token_expires_in = (int) $data['expires_in'];
	}
