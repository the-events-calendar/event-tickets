...
export function updateSeriesData( uneditableTickets = [] ) {
	...
	const seriesPasses = uneditableTickets.filter( ( ticket ) => ticket.type === 'series_pass' );
	...
	const independentCapacity = independent.reduce( ( acc, ticket ) => acc + ( ticket?.capacity || 0 ), 0 );
	const sharedCapacity = shared.reduce( ( acc, ticket ) => Math.max( acc, ticket?.capacity || 0 ), 0 );
	const independentAvailable = independent.reduce( ( acc, ticket ) => acc + ( ticket?.available || 0 ), 0 );
	const sharedAvailable = shared.reduce( ( acc, ticket ) => Math.max( acc, ticket?.available || 0 ), 0 );
	const hasUnlimitedSeriesPasses = Boolean( unlimited.length );
	const seriesPostId = getSeriesPostIdFromSelection();
	const seriesPlainUrl = sprintf( ftStore.getSeriesHeaderLinkTemplate(), seriesPostId );
	...
	setSeriesData( isInSeries, {
		...
		seriesPassIndependentCapacityItems: independent.map( ( ticket ) => ticket?.title ).join( ', ' ),
		seriesPassSharedCapacityItems: shared.map( ( ticket ) => ticket?.title ).join( ', ' ),
		seriesPassUnlimitedCapacityItems: unlimited.map( ( ticket ) => ticket?.title ).join( ', ' ),
...
addAction( 'tec.tickets.blocks.uneditableTicketsUpdated', 'tec.tickets.flexibleTickets', updateSeriesData );
