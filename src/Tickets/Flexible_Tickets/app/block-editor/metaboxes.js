import { subscribe as wpDataSubscribe, select as wpDataSelect } from '@wordpress/data';
...
const updateUneditableTicketsOnMetaboxUpdate = function () {
	const isSavingMetaBoxes = wpDataSelect( 'core/edit-post' ).isSavingMetaBoxes();
	...
	if ( wasSavingMetaBoxes !== isSavingMetaBoxes ) {
		if ( ! isSavingMetaBoxes ) {
			...
			onMetaBoxesUpdateCompletedCallbacks.forEach( ( callback ) => callback() );
 ...
 */
const subscribeToMetaBoxesUpdates = () => wpDataSubscribe( updateUneditableTicketsOnMetaboxUpdate );
