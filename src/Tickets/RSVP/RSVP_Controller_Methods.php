...
trait RSVP_Controller_Methods {
	 ...
	 * @return void
	 */
	protected function register_common_rsvp_implementations(): void {
	 ...
	 * @return void
	 */
	protected function register_csv_importer_hooks(): void {
	 ...
	 * @return void
	 */
	protected function register_rsvp_hooks( Tribe__Tickets__RSVP $rsvp ): void {
		...
		add_filter(
			...
			[ $rsvp, 'filter_event_tickets_attendees_rsvp_checkin_stati' ] 
		);
	 ...
	 * @return void
	 */
	protected function register_promoter_hooks(): void {
		// RSVP Observer hooks - store callbacks for unregistration.
		$this->callbacks['rsvp_observer_checkin']          = $this->container->callback(
			RSVP_Observer::class,
			'rsvp_checkin' 
		);
		$this->callbacks['rsvp_observer_attendee_created'] = $this->container->callback(
			RSVP_Observer::class,
			'attendee_created' 
		);
		$this->callbacks['rsvp_observer_attendee_updated'] = $this->container->callback(
			RSVP_Observer::class,
			'attendee_updated' 
		);
		...
		$this->callbacks['promoter_notify_ticket_event']       = $this->container->callback(
			Promoter_Observer::class,
			'notify_ticket_event' 
		);
		$this->callbacks['promoter_notify_event_id_deleted']   = $this->container->callback(
			Promoter_Observer::class,
			'notify_event_id' 
		);
		$this->callbacks['promoter_notify_event_id_generated'] = $this->container->callback(
			Promoter_Observer::class,
			'notify_event_id' 
		);
		...
		add_action(
			...
			$this->callbacks['promoter_notify_event_id_generated'],
			10,
			2 
		);
	}
	 ...
	 * @return void
	 */
	protected function register_block_hooks( Blocks_RSVP $rsvp_block ): void {
	...
	public function unregister(): void {
		...
		if ( isset( $this->callbacks['rsvp_observer_attendee_created'] ) ) {
			remove_action(
				...
				$this->callbacks['rsvp_observer_attendee_created'],
				10 
			);
		}
