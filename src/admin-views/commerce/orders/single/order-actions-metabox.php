	...
	<div id="major-publishing-actions">
		...
		<div id="delete-action">
			...
			if ( current_user_can( 'delete_post', $order->ID ) ) {
				if ( ! EMPTY_TRASH_DAYS ) {
					$delete_text = __( 'Delete permanently', 'event-tickets' );
				} else {
					$delete_text = __( 'Move to Trash', 'event-tickets' );
				}
				?>
				<a class="submitdelete deletion" href="<?php echo esc_url( get_delete_post_link( $order->ID ) ); ?>"><?php echo esc_html( $delete_text ); ?></a>
				<?php
			}
			?>
		</div>
