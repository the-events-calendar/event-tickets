...
1. **PHP Side**: The PHP code (e.g., in Landing Page classes) outputs a `<script>` tag in the `<head>` that sets `window.tecTicketsWebpackPublicPath` to the correct URL
...
**Note**: We use a namespaced variable (`window.tecTicketsWebpackPublicPath`) to avoid polluting the global namespace, but must assign it to webpack's special `__webpack_public_path__` variable which webpack specifically looks for.

### Important Notes

- This must be imported **before** any modules that import assets
- The corresponding PHP code must set `window.tecTicketsWebpackPublicPath` via `admin_head` or similar hook
