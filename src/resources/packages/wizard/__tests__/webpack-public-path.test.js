...
describe( 'webpack-public-path', () => {
	...
	it( 'should set __webpack_public_path__ when window.tecTicketsWebpackPublicPath is defined', () => {
		// Arrange
		const expectedPath =
			'https://example.com/wp-content/plugins/event-tickets/build/';
		global.window.tecTicketsWebpackPublicPath = expectedPath;
		...
		expect( global.window.tecTicketsWebpackPublicPath ).toBe(
			expectedPath
		);
	} );

	it( 'should not throw error when window.tecTicketsWebpackPublicPath is undefined', () => {
		// Arrange
		delete global.window.tecTicketsWebpackPublicPath;
	...
	it( 'should work with various URL formats', () => {
		...
		testCases.forEach( ( url ) => {
			...
			// Arrange
			global.window = { tecTicketsWebpackPublicPath: url };
			...
			expect( global.window.tecTicketsWebpackPublicPath ).toBe( url );
		} );
	} );
	...
	it( 'should preserve trailing slash in path', () => {
		...
		global.window.tecTicketsWebpackPublicPath = pathWithSlash;

		// Act
		require( '../webpack-public-path' );

		// Assert
		expect( global.window.tecTicketsWebpackPublicPath ).toMatch( /\/$/ );
	} );

	it( 'should handle paths with special characters', () => {
		// Arrange
		const specialPath =
			'https://example.com/my-site/wp-content/plugins/event-tickets/build/';
		global.window.tecTicketsWebpackPublicPath = specialPath;
	...
	it( 'should be importable before other modules', () => {
		...
		// Arrange
		global.window.tecTicketsWebpackPublicPath =
...
describe( 'webpack-public-path integration', () => {
	...
	it( 'should work when imported at the top of an entry point', () => {
		// Arrange
		global.window.tecTicketsWebpackPublicPath = 'https://test.com/build/';
