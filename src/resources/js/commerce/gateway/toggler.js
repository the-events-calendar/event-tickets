...
(
	( $, obj ) => {
		...
		obj.showGateway = ( gateway ) => {
			$( gateway ).show().attr( 'aria-expanded', 'true' ).removeClass( obj.selectors.toggleHidden.className() );
		};
		...
		obj.hideGateway = ( gateway ) => {
			$( gateway ).hide().attr( 'aria-expanded', 'false' ).addClass( obj.selectors.toggleHidden.className() );
		};
		...
		obj.showAdditional = () => {
			...
			obj.toggles.default
				.removeClass( obj.selectors.toggleOpen.className() )
				.show()
		...
		obj.getDefaultToggleHTML = () => {
			return (
				`<div class="${ obj.selectors.toggle.className() }">` +
