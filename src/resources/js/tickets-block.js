...
( function ( $, obj, tde ) {
	...
	obj.updateAvailability = function ( tickets ) {
		Object.keys( tickets ).forEach( function ( ticketId ) {
			...
			if ( 1 < available ) {
				// Ticket in stock, we may want to update values.
	 ...
	 *
	 * @return {Object} - Returns the updated item for chaining.
	 */
	...
	obj.maybeShowNonMetaNotice = function ( $form ) {
		let nonMetaCount = 0;
		let metaCount = 0;
		const $cartItems = $form.find( obj.selector.item ).filter( function () {
	 ...
	 * @since 4.9
	 *
	 * @return {number[]} Array of tickets IDs.
	 */
	obj.getTickets = function () {
		let tickets = $( obj.selector.item )
			.map( function () {
		 ...
		 */
		tickets = applyFilters( 'tec.tickets.tickets-block.getTickets', tickets );
	...
	obj.appendARFields = function ( $form ) {
		$form.find( obj.selector.item ).each( function () {
			const $cartItem = $( this );
	...
	obj.checkAvailability = function () {
		...
		$.post( TribeTicketOptions.ajaxurl, params, function ( response ) {
	 ...
	 * @param {Object} $cartItem - The cart item to update.
	 *
	 * @return {number} - The item quantity.
	 */
	...
	obj.initModalFormPrefills = function () {
		...
		$.when( obj.getData() ).then( function ( data ) {
	...
	obj.maybeHydrateAttendeeBlockFromLocal = function ( length ) {
		$.when( obj.getData() ).then( function ( data ) {
			...
			$newBlocks.find( obj.modalSelector.metaField ).each( function () {
				const $this = $( this );
	 ...
	 *
	 * @return {Object} - Deferred data object.
	 */
	 ...
	 *
	 * @return {boolean} - Is checkbox valid?
	 */
	 ...
	 * @since 4.9
	 */
	obj.document.on( 'change keyup', obj.selector.itemQuantityInput, function ( e ) {
		const $this = $( e.target );
	 ...
	 * @since 4.11.0
	 */
	obj.document.on( 'beforeunload', function () {
	...
	obj.document.on( 'keypress', obj.modalSelector.form, function ( e ) {
		if ( e.keyCode === 13 ) {
	 ...
	 * @since 4.11.0
	 */
	obj.document.on( 'click', obj.selector.classicSubmit, function ( e ) {
		e.preventDefault();
	 ...
	 * @since 4.11.4
	 */
	obj.document.on( 'keypress', obj.selector.itemQuantityInput, function ( e ) {
		if ( e.keyCode === 13 ) {
			e.preventDefault();
			e.stopPropagation();
		}
	 ...
	 * @since 4.11.0
	 */
	$( tde ).on( 'tribe_dialog_show_ar_modal', function () {
		obj.loaderShow();
		obj.loaderShow( obj.modalSelector.loader );
	 ...
	 */
	$( tde ).on( 'tribe_dialog_close_ar_modal', function () {
		obj.storeLocal();
	} );
