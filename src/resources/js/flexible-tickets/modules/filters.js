...
function changeTicketTypeDescriptionForEventPartOfSeries( mappedProps, { ticketDetails } ) {
    ...
    const newDescription = TECFtEditorData?.defaultTicketTypeEventInSeriesDescription;
 ...
 *
 * @returns {Object} The modified properties mapped from the state for the Tickets component.
 */
 ...
 *
 * @returns {Object} The modified properties mapped from the state for the TicketsDashboardAction component.
 */
function filterTicketsDashboardActionsMappedProps( mappedProps, { isRecurring } ) {
    ...
    const hasSeriesPasses = ( TECFtEditorData?.series?.seriesPassesCount || 0 ) > 0
...
function filterTicketsAvailabilityMappedProps( mappedProps ) {
    ...
    const seriesCapacity = TECFtEditorData?.series?.seriesPassTotalCapacity || 0;
    const seriesAvailability = TECFtEditorData?.series?.seriesPassAvailableCapacity || 0;
    const isInSeries = tecEventDetails?.isInSeries;
...
function filterTicketsControlsMappedProps( mappedProps, { isRecurring = false } ) {
    const isInSeries = tecEventDetails?.isInSeries;
...
/**
 *
 * Forces the Prompt Message component to show on Recurring Events if they are part of a Series.
 *
 * @since TBD
 *
 ...
 * @returns
 */
function filterDisplaySeriesPassesPromptMessage( component ) {
	const hasSeriesPasses = ( TECFtEditorData?.series?.seriesPassesCount || 0 ) > 0;
...
wp.hooks.addFilter(
	...
	'tec.tickets.flexibleTickets',
	filterDisplaySeriesPassesPromptMessage,
)